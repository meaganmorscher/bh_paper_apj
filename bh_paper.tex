...
$\sim 5-30\, M_\odot$ for $Z=0.001$. Both neutron stars and BHs receive natal kicks
assumed to be generated by the asymmetric ejection of mass during 
a supernova explosion. Neutron star kicks are drawn from a Maxwellian
...
While it is thought that clusters form with primordial binaries, three-body binary
formation is expected to produce an important population of hard BH-binaries 
...
We have recently 
implemented a simplified prescription for three-body binary formation which allows
 BHs to form binaries through an interaction involving three single BHs.
If three single stars experience a close resonant encounter, it is possible for two
of the stars to become gravitationally bound to one another, with their binding
energy being balanced by a gain in kinetic energy of the third star. The probability
...
to collapse by a population of BHs.

We restrict our attention to 
dynamically hard binaries, as only hard binaries are expected to survive \citep{Heggie1975}.
We only allow BHs to form binaries, since they are expected to be found near the cluster center
where the density is high enough for binary formation to occur.
Our simplified prescription involves calculating the rate at which 
...
and the current timestep, we can estimate the probability that the three-body
 system will produce a binary. Then we use MC sampling to select which systems
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    FIGURES    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% central lum density histogram Ð GCs + models 
% cluster mass histogram Ð GCs + models 
% core radii histogram Ð GCs + models 
% rh/rc histogram Ð GCs + models 
% Rc vs Mcl Ð GCs + models
%
...
\begin{figure}
	...
	\caption{Histogram of observed core radii for Milky Way GCs (Harris), along with the core radii measured from our standard models, represented by tick marks on the x-axis. Red and blue ticks correspond to models with $N=2 \times 10^5$ and $N=8 \times 10^5$, respectively. Our models produce core radii that are significantly larger than most of the MW clusters. We do not produce any models with $r_c \leq 1$ pc
	}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    TABLES    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1. Initial properties Ð all models
% 2. Final properties Ð all models
% 3. Final BH stats Ð all models
